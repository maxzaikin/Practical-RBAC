Function Delegate-UA-Edit-EmployeeType
{
    <#
        .SYNOPSIS
            Delegates "Edit EmployeeType" permissions to an Active Directory group.
        ...
        .EXAMPLE
            Delegate-UA-Edit-EmployeeType -role_name 'HR_Team' -ou 'OU=Standard Users,DC=contoso,DC=com' 

        .NOTES
            This function delegates "Edit EmployeeType" permissions to a specified Active Directory group for a specific OU.
            It allows the group members to modify the `EmployeeType` attribute within the designated OU.
    ...
    BEGIN { 
        ...
        # GUID for the EmployeeType attribute in Active Directory.
        $EmployeeTypeProperty = [GUID]"bf9679c6-0de6-11d0-a285-00aa003049e2"
        ...
        $new_note = "Edit EmployeeType permissions have been delegated to RBAC role $($role_name) on OU=$($ou). Operator: $($current_user), computer: $($computerName). Date:$($current_date)"        
    }
        ...
        {  
            if ($WhatIf) {
                ...
                Write-Verbose "$($ou_acl).AddAccessRule((New-Object System.DirectoryServices.ActiveDirectoryAccessRule $($ACE_TRUSTEE),
                                                                                                                       ...
                                                                                                                       $($AccessControlAllow),
                                                                                                                       $($EmployeeTypeProperty),
                                                                                                                       $($InheritanceFlags),
                                                                                                                       $($AD_CLASS_USER_OBJECT)))"
            }
            else {
                # Create a new access rule granting the role permission to edit EmployeeType.
