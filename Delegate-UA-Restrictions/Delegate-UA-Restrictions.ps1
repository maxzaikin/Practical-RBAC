Function Delegate-UA-Restrictions {
    <#
        .SYNOPSIS
            Delegates User Account Restrictions Information permissions to an Active Directory group.
        ...
        .NOTES
            This function delegates "Account Restrictions" permissions to a specified Active Directory group for a specific OU.
            It allows the group members to manage the account expiration date within the designated OU.
    ...
    BEGIN { 
        ...
        $new_note = "Directory services access information permissions have been delegated to RBAC role $($role_name) on OU=$($ou). Operator: $($current_user), computer: $($computerName). Date:$($current_date)"        
          
        # Define the GUID for the "Account Expires" attribute in Active Directory.
        $AccountRestrictionsProperty = [GUID]"4C164200-20C0-11D0-A768-00AA006E0529" 
        ...
        # Define the inheritance flag: Self and Children.
        ...
        {  
            ...
            if ($WhatIf) {
                ...
                Write-Verbose "$($ou_acl).AddAccessRule((New-Object System.DirectoryServices.ActiveDirectoryAccessRule $($ACE_TRUSTEE),
                                                                                                                       ...
                                                                                                                       $($AccessControlAllow),
                                                                                                                       $($AccountRestrictionsProperty),
                                                                                                                       $($InheritanceFlags),
                                                                                                                       $($AD_CLASS_USER_OBJECT)))"
            }
            else {
                # Create a new access rule granting the role permission to manage Account Expires.
