Function Delegate-UA-Edit-DisplayName
{
    <#
        .SYNOPSIS
            Delegates "Edit DisplayName" permissions to an Active Directory group.
        ...
        .EXAMPLE
            Delegate-UA-Edit-DisplayName -role_name 'Helpdesk_Tier1' -ou 'OU=Standard Users,DC=contoso,DC=com' 

        .NOTES
            This function delegates "Edit DisplayName" permissions to a specified Active Directory group for a specific OU.
            It allows the group members to modify the `DisplayName` attribute within the designated OU.
    ...
    BEGIN { 
        ...
        # GUID for the DisplayName attribute in Active Directory.
        $DisplayNameProperty = [GUID]"bf9679e7-0de6-11d0-a285-00aa003049e2"
        ...
        $new_note = "Edit DisplayName permissions have been delegated to RBAC role $($role_name) on OU=$($ou). Operator: $($current_user), computer: $($computerName). Date:$($current_date)"        
    }
    ...
    PROCESS {
        ...
        {  
            if ($WhatIf) {
                ...
                # Create a new access rule granting the role permission to edit DisplayName.
