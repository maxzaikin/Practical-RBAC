Function Delegate-UA-Edit-Public-Information {
    <#
        .SYNOPSIS
            Delegates "Edit Basic User Attributes such as public information" permissions to an Active Directory group.
        ...
        .NOTES
            This function delegates permissions to edit basic user attributes to a specified Active Directory group for a specific OU.
            It allows the group members to edit attributes like first name, last name, phone number, etc. within the designated OU.
    ...
    BEGIN { 
        ...
        $new_note = "Basic User Attributes editing permissions have been delegated to RBAC role $($role_name) on OU=$($ou). Operator: $($current_user), computer: $($computerName). Date:$($current_date)"        
          
        # Define the GUID for the "Personal Information" property set in Active Directory.
        ...
        {  
            ...
            if ($WhatIf) {
                ...
                Write-Verbose "$($ou_acl).AddAccessRule((New-Object System.DirectoryServices.ActiveDirectoryAccessRule $($ACE_TRUSTEE),
                                                                                                                       ...
                                                                                                                       $($AccessControlAllow),
                                                                                                                       $($PublicInformationPropertySet),
                                                                                                                       $($InheritanceFlags),
                                                                                                                       $($AD_CLASS_USER_OBJECT)))"
            }
            else {
                # Create a new access rule granting the role permission to edit the Personal Information property set.
